  ...
  return (
    <div className="flex flex-col w-full">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full">
        {filtered
          ...
          .map(({ title, image, lprice, mallName, id }, i) => (
            <Link
              key={id}
              className="flex flex-col w-full bg-mono_000 rounded-xl shadow hover:shadow-lg transition overflow-hidden"
              href={`/shop/${id}`}
            >
              ...
              <div className="flex flex-col justify-between p-4 flex-1">
                ...
                <div className="mt-4">
                  ...
                  <div className="flex justify-end">
                    <button className="w-full h-[37px] rounded-md bg-main text-white text-sm hover:opacity-90 transition">
                      구매하러가기
                    </button>
                  </div>
                </div>
      ...
      {hasMore && filtered.length > page * PRODUCTS_PER_PAGE && (
        <div className="flex justify-center w-full pt-10 pb-[100px]">
          <button
            className="w-[290px] h-[37px] rounded-md bg-main text-white text-sm hover:opacity-90 transition"
            disabled={loading}
